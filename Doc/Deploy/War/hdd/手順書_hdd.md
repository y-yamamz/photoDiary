...
写真ファイルの保存先は **外部HDD上のフォルダ（ローカルバインドマウント）** を使用します。
...
ブラウザ → Nginx (port 80) ─┬─ 静的ファイル (React)
                             ├─ /api/**    → Tomcat (8080)
                             └─ /images/** → Tomcat (8080) → 外部HDD (Y:\PhotoDiary)
```

> **バインドマウントの仕組み**
> `docker-compose.hdd.yml` では Docker のバインドマウントを使い、
> ホスト上のフォルダをコンテナ内の `/var/photodiary/images` に直接マウントします。
> **マウント先フォルダは事前に作成しておく必要があります**（Docker は自動作成しません）。
...
Doc/Deploy/War/hdd/
├── 手順書_hdd.md              # 本ファイル
├── docker-compose.hdd.yml     # Docker Compose 定義 (外部HDD版)
...
**Docker のバインドマウントはフォルダが存在しないとエラーになります。** 必ず事前に作成してください。

```cmd
mkdir Y:\PhotoDiary
```

> 別のドライブ・パスに保存する場合は `.env` の `IMAGE_LOCAL_PATH` を実際のパスに変更してください。
> 例: `IMAGE_LOCAL_PATH=D:/PhotoDiary`

### 2. Docker Desktop の File sharing 設定

`Y:` ドライブ等のカスタムパスを使う場合は Docker Desktop への登録が必要です。

1. タスクトレイの Docker アイコンを右クリック → **Settings** を開く
2. **Resources** → **File sharing** を選択
3. **＋** ボタンをクリックして対象フォルダを追加
...
```cmd
cd D:\WORK\study\vscode\gitHub\photoDiary\Doc\Deploy\War\hdd
```
...
MYSQL_PASSWORD=yama
FRONTEND_PORT=80
IMAGE_LOCAL_PATH=Y:/PhotoDiary
```

> **パスの記載形式について**
> バックスラッシュ (`Y:\PhotoDiary`) ではなく **スラッシュ** (`Y:/PhotoDiary`) で記載してください。
> Docker Compose の変数展開でバックスラッシュがエスケープ文字として解釈されるためです。
...
```cmd
D:\WORK\study\vscode\gitHub\photoDiary\Doc\Deploy\War\hdd\build-war.bat
```
...
| [2/4] | `backend/build/libs/backend.war` を `hdd/` フォルダへコピー |
| [3/4] | `frontend/` で `npm run build` を実行 |
| [4/4] | `create-frontend-war.ps1` を呼び出し `frontend/dist/` を `frontend.war` にパッケージ |

完了すると以下が生成されます：
```
Doc/Deploy/War/hdd/backend.war
Doc/Deploy/War/hdd/frontend.war
```
...
```cmd
cd D:\WORK\study\vscode\gitHub\photoDiary\Doc\Deploy\War\hdd
docker compose -f docker-compose.hdd.yml up -d --build
```
...
```cmd
docker compose -f docker-compose.hdd.yml ps
```
...
:: 2. コンテナを再ビルド・再起動
cd D:\WORK\study\vscode\gitHub\photoDiary\Doc\Deploy\War\hdd
docker compose -f docker-compose.hdd.yml down
docker compose -f docker-compose.hdd.yml up -d --build
```
...
docker compose -f docker-compose.hdd.yml down
docker rmi hdd-frontend
docker compose -f docker-compose.hdd.yml up -d --build

:: バックエンドのみ（確実に再ビルドする場合）
docker compose -f docker-compose.hdd.yml down
docker rmi hdd-backend
docker compose -f docker-compose.hdd.yml up -d --build
```
...
docker compose -f docker-compose.hdd.yml down --rmi all
```
...
```env
IMAGE_LOCAL_PATH=Y:/PhotoDiary   # スラッシュ区切りで記載
```

**3. 外付けドライブ使用時は Docker Desktop の File sharing を確認**
- Docker Desktop → Settings → Resources → File sharing でドライブが登録されているか確認
...
外部HDD を抜き差しするとドライブレターが変わる場合があります。
...
ドライブレターが変わった場合は `.env` の `IMAGE_LOCAL_PATH` を修正してください。
また Docker Desktop の File sharing 設定も更新が必要な場合があります。
...
```cmd
:: 全コンテナのログ（リアルタイム）
docker compose -f docker-compose.hdd.yml logs -f
